...
//		System.out.println("后缀表达式：" + Postfix);
		...
		for (int i = 0; i < s.length; i++) {
			if(s[i].matches("-?\\d+")){
				...
				bean.setNumberB(Integer.valueOf(stack.pop()));
				bean.setNumberA(Integer.valueOf(stack.pop()));
				bean.setSymbol(s[i]);
				oper = OperationFactory.createOperate(bean);
				oper.calculate();
				stack.push(String.valueOf(bean.getResult()));
			}
		}
		return (stack.pop());
